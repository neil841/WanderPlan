...
import type { LandingPage, LandingPageBlock, LandingPageBlockType } from '@/types/landing-page';

interface PageEditorProps {
  landingPage: LandingPage;
}

export function PageEditor({ landingPage }: PageEditorProps) {
  const router = useRouter();
  const updateMutation = useUpdateLandingPage(landingPage.slug);

  // Local state for editing
  const [title, setTitle] = useState(landingPage.title);
  const [description, setDescription] = useState(landingPage.description || '');
  const [blocks, setBlocks] = useState<LandingPageBlock[]>(landingPage.content.blocks);
          ...
          <div className="flex items-center gap-2">
            ...
            {isPublished && (
              <Button
                ...
                asChild
              >
                <a href={`/p/${landingPage.slug}`} target="_blank" rel="noopener noreferrer">
